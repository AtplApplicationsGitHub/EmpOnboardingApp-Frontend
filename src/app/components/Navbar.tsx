...
import { Home, Users, HelpCircle, LogOut, UserPlus, Settings, FileText } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-card border-r border-border ${isVisible ? animationClasses.slideInLeft : 'opacity-0'}`}>
      <div className="flex flex-col h-full">
        {/* Logo/Brand */}
        <div className="flex items-center h-16 px-6 border-b border-border">
          <Link href="/" className="flex items-center space-x-2">
            <span className="font-bold text-xl text-foreground">Onboarding App</span>
          </Link>
        </div>
        
        {/* Navigation */}
        <nav className="flex-1 px-4 py-6">
          <div className="space-y-2">            {user.role === 'admin' && (
              <>
                <Link 
                  href="/admin" 
                  className={cn(
