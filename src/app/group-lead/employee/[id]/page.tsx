...
const EmployeeTasksPage: React.FC = () => {
  ...
  const autoSaveResponse = async (taskId: number, response: string) => {
    ...
    try {
      ...
      const updatedTask = await groupLeadService.saveTaskResponse(taskId, response);
      ...
      console.log('Auto-save successful:', { taskId });
  ...
  const getCurrentResponse = (task: Task): string => {
    return responses[task.id] ?? task.response ?? '';
  };
                ...
                {currentTasks.map((task, index) => {
                  ...
                  return (
                    <TableRow key={task.id} className="hover:bg-muted/50">
                      ...
                      {/* Status Flag */}
                      <TableCell>
                        <div className={`flex items-center gap-2 px-2 py-1 rounded-full text-xs font-medium ${statusFlag.bg}`}>
